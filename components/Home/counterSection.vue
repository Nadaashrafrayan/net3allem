<template>
  <section class=" flex flex-col items-center justify-center text-center md:px-4 pb-4 px-14 text-grey20">
    ...
    <h3 class="md:text-[30px] mt-8 py-6 text-[27px] font-extrabold leading-9 font-sans uppercase text-ourOrange tracking-[0.2em]">Sponsor a student for:
    ...
    <div class="md:space-x-4 space-x-0 space-y-2 md:space-y-0 flex md:flex-row flex-col">
      ...
      <button
        ...
        class="md:px-6 px-10 py-2 bg-OurGreen hover:border-[1px] hover:text-ourGrey hover:bg-transparent text-white font-semibold rounded-[10px]  "
      >
        Year ( 8400 EGP)
      </button>
    </div>
...
export default {
  ...
  methods: {
    startCounter() {
      ...
      const increment = Math.ceil(this.target / 100); // Adjust speed based on the target
    ...
    sponsorOption(option) {
      if (option === 'month') {
        alert('You have chosen to sponsor a student for 700 EGP per month.');
      } else if (option === 'year') {
        alert('You have chosen to sponsor a student for 8400 EGP per year.');
      }
    },
  },
  beforeDestroy() {
